        ...
        while l[0:3] != "TER" and l[0:6] != "CONECT" and l[21] == chain_letter:
            if l[0:4] == "ATOM":
                ...
                while l[0:3] != "TER" and l[22:26] == res_number:
                    if l[0:4] == "ATOM":
                        if opdracht == "RMSF" or opdracht == "RDF":
...
def WriteRDF():
    ...
    f.write("@prefix : <http://biobeat.org/rdf/ns#> .\n")
    for m in models:
        f.write(":Model%.0f\n" %m.number)
        n = 0
        for c in m.chains:
            n = n +1
            if n == len(m.chains):
                f.write("\t:Chain%s :m%.0f_%s .\n" %(c.letter, m.number, c.letter))
            else:
                f.write("\t:Chain%s :m%.0f_%s ;\n" %(c.letter, m.number, c.letter))
        for c in m.chains:
            f.write(":m%.0f_%s\n" %(m.number, c.letter))
            ...
            for r in c.res:
                ...
                for a in r.atoms:
                    o = o + 1
                    if t == len(c.res) and o == len(r.atoms):
                        f.write("\t:Atom%.0f :m%.0f_%s_%.0f .\n" %(a.number, m.number, c.letter, a.number))
                    else:
                        f.write("\t:Atom%.0f :m%.0f_%s_%.0f ;\n" %(a.number, m.number, c.letter, a.number))
            for r in c.res:
                for a in r.atoms:
                    f.write(":m%.0f_%s_%.0f\n" %(m.number, c.letter, a.number))
#                    f.write("\t:name :%s ;\n" %a.name)
                    f.write("\t:x %.3f ;\n" %a.x)
                    f.write("\t:y %.3f ;\n" %a.y)
                    f.write("\t:z %.3f ;\n" %a.z)
